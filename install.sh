...
# التحقق مما إذا كنا نعمل من النسخة المحلية أم البعيدة
# إذا كان المتغير GT_LOCAL_INSTALLER موجوداً، فهذا يعني أننا نعمل من النسخة المحلية
if [ -n "$GT_LOCAL_INSTALLER" ]; then
    # نحن في النسخة المحلية، نكمل التنفيذ الطبيعي
    :
else
    # نحن في النسخة البعيدة، نقوم بتنزيل الملفات ثم نستدعي المحلي مع تعيين المتغير
    # هذا يمنع التكرار اللانهائي
    ...
    if [ "$SYSTEM_INSTALLED" = true ] || [ "$LOCAL_INSTALLED" = true ]; then
        ...
        case $prev_choice in
            1)
                ...
                # متابعة إلى عملية التحميل والتثبيت
                return 0
                ;;
            2)
...
echo ""
print_color "$CYAN" "════════════════════════════════════════════════════════════"
print_color "$CYAN" "           🛠️  GT-customterminal Installer v1.0.9"
...
# إذا كنا نعمل من النسخة المحلية، نقوم بالتحقق من التثبيت السابق والمتابعة
if [ -n "$GT_LOCAL_INSTALLER" ]; then
    # التحقق من التثبيت السابق
    check_previous_installation

    # الاستمرار في عملية التثبيت العادية
    # ============================================
    # متابعة التثبيت العادي (تحميل الملفات إذا لزم الأمر)
    # ============================================
    ...
    # التحقق مما إذا كنا بحاجة لتنزيل الملفات (إذا كان المجلد فارغاً)
    if [ ! -f "gt-customterminal.sh" ] || [ ! -f "install.sh" ]; then
        ...
        echo ""
        print_success "Files already exist locally. Skipping download."
    fi
    ...
    # نحن في النسخة البعيدة - نقوم بتنزيل الملفات ثم نستدعي المحلي
    ...
    # تشغيل المثبت المحلي مع تعيين متغير البيئة لمنع التكرار
    echo "────────────────────────────────────────────────────────────"
